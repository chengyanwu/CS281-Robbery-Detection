...
class Trainer:
    ...
    def build_act(self, transformer):
        ...
        outputs = tf.keras.layers.Dense(
            self.config[self.config['DATASET']]['CLASSES'])(x)
        return tf.keras.models.Model(inputs, outputs)

    def get_model(self):
        transformer = TransformerEncoder(
            self.d_model, self.n_heads, self.d_ff, self.dropout, self.activation, self.n_layers)
        self.model = self.build_act(transformer)

        self.train_steps = np.ceil(
            float(self.train_len)/self.config['BATCH_SIZE'])
        self.test_steps = np.ceil(
            float(self.test_len)/self.config['BATCH_SIZE'])
    ...
    def get_data(self):
        if self.config['DATASET'] == 'kinetics':
            ...
            total_number_of_steals = 0
            total_number_of_non_steals = 0

            # List all the directories in the given path
            # with os.scandir('/home/homesecurity/CS281-Robbery-Detection/test/examples/res/') as entries:
            ...
            #             elif entry.name.startswith('non-steal'):
            #                 total_number_of_non_steals += 1

            for k in range(1, 404):
                file_path: str = f'/home/homesecurity/CS281-Robbery-Detection/test/examples/res/steal{k}/alphapose-results.json'
            ...
            for k in range(1, 384):
                file_path: str = f'/home/homesecurity/CS281-Robbery-Detection/test/examples/res/non-steal{k}/alphapose-results.json'
                if os.path.isfile(file_path):
                    with open(file_path, "r") as f:
                        data: List[Dict[str, Any]] = json.load(f)
                    if(len(data) != 0):
            ...
            new_kp_seq: List[List[List[float]]] = [[]
                                                   for _ in range(total_number_of_videos)]
            
            print('--------test--------')
            print(total_number_of_non_steals)
            print(total_number_of_steals)
                ...
                if os.path.isfile(file_path):
                    ...
                    if(len(data) != 0):
                        ...
                        for i in range(30):
                            ...
                            # first k == 1
                            # last k == 135 => k-1 == 134
                            new_kp_seq[cnt].append(extended_kp_seq[int(idx)])
                        cnt+=1
                else:
                    print("keypoint json file not existed!")
                ...
                if os.path.isfile(file_path):
                    ...
                    if(len(data) != 0):
                        ...
                        for i in range(30):
                            idx = int(i * multiplier)
                            # start from 134
                            new_kp_seq[total_number_of_non_steals-1+cnt].append(extended_kp_seq[int(idx)])
                        cnt += 1
                else:
                    print("Keypoint json file not found!")

                
        
            total_n = total_number_of_videos
            new_n = len(new_kp_seq) - old_n
            print('new input size: ', total_n)
    ...
    def get_random_hp(self):
        ...
        self.config['WEIGHT_DECAY'] = self.trial.suggest_discrete_uniform(
            "WD", 1e-5, 1e-3, 1e-5)
        self.config['N_EPOCHS'] = int(
            self.trial.suggest_discrete_uniform("EPOCHS", 50, 100, 10))
        self.config['WARMUP_PERC'] = self.trial.suggest_discrete_uniform(
            "WARMUP_PERC", 0.1, 0.4, 0.1)
        self.config['LR_MULT'] = self.trial.suggest_discrete_uniform(
            "LR_MULT", -5, -4, 1)
        self.config['SUBSAMPLE'] = int(
            self.trial.suggest_discrete_uniform("SUBSAMPLE", 4, 8, 4))
        self.config['SCHEDULER'] = self.trial.suggest_categorical("SCHEDULER", [
                                                                  False, False])

        # self.logger.save_log('\nRN_STD: {:.2e}'.format(self.config['RN_STD']))
        self.logger.save_log('\nEPOCHS: {}'.format(self.config['N_EPOCHS']))
        self.logger.save_log('WARMUP_PERC: {:.2e}'.format(
            self.config['WARMUP_PERC']))
        self.logger.save_log('WEIGHT_DECAY: {:.2e}'.format(
    ...
    def do_training(self):
        ...
        self.model.fit(self.ds_train,
                       ...
                       callbacks=[
                           self.checkpointer], verbose=self.config['VERBOSE'],
        ...
        _, accuracy_test = self.model.evaluate(
            self.ds_test, steps=self.test_steps)
        ...
        print("-----------")
        y_pred = np.argmax(tf.nn.softmax(self.model.predict(
            tf.concat(X, axis=0)), axis=-1), axis=1)
    ...
    def do_benchmark(self):
        for split in range(1, self.config['SPLITS']+1):
            ...
            np.save(self.config['RESULTS_DIR'] + self.config['MODEL_NAME'] +
                    '_' + self.config['DATASET'] + f'_{split}_accuracy.npy', acc_list)
            np.save(self.config['RESULTS_DIR'] + self.config['MODEL_NAME'] + '_' +
                    self.config['DATASET'] + f'_{split}_balanced_accuracy.npy', bal_acc_list)
            ...
            self.logger.save_log(
                f"Balanced Accuracy mean: {np.mean(bal_acc_list)}")
            self.logger.save_log(
                f"Balanced Accuracy std: {np.std(bal_acc_list)}")
